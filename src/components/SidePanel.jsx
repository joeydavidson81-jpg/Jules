...
// ── Device ID (persists across sessions, no login needed) ─────────────────────
...
function EventCard({ event, facilityId }) {
  ...
  // signups is an array of {id, name} objects
  const signups     = Array.isArray(event.signups) ? event.signups : []
  const mySignup    = signups.find((s) => s.id === deviceId)
  const signedUp    = Boolean(mySignup)
  ...
  const handleSignup = useCallback(async () => {
    ...
    try {
      ...
      await updateDoc(ref, { signups: arrayUnion({ id: deviceId, name: name.trim() }) })
      setShowNameForm(false)
      setName('')
    } finally {
      setBusy(false)
    }
  }, [deviceId, facilityId, event.id, name])
  ...
  const handleWithdraw = useCallback(async () => {
    ...
    try {
      const ref = doc(db, 'facilities', facilityId, 'events', event.id)
      await updateDoc(ref, { signups: arrayRemove(mySignup) })
    } finally {
      setBusy(false)
    }
  }, [mySignup, facilityId, event.id])

  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
      {/* Title */}
      <h4 className="text-sm font-semibold text-gray-900 leading-tight mb-1">{event.title}</h4>
      ...
      {/* Signup count (when no cap set) */}
      {needed === null && signupCount > 0 && (
        <p className="text-xs text-gray-400 mb-2">
          {signupCount} volunteer{signupCount !== 1 ? 's' : ''} signed up
        </p>
      )}

      {/* Name form (shown when volunteer clicks Sign Up) */}
      {showNameForm && (
        <div className="flex gap-2 mb-2">
          ...
          <button
            onClick={() => { setShowNameForm(false); setName('') }}
      ...
      {/* Action button — only shown to non-admins */}
      {!canManage && !showNameForm && (
      ...
      {/* Volunteer names visible to admin */}
      {canManage && signups.length > 0 && (
        <div className="mt-2 pt-2 border-t border-gray-100">
          <p className="text-xs font-semibold text-gray-500 mb-1">Signed up:</p>
...
export default function SidePanel({ facilityId, onClose }) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/20 z-[900] lg:hidden" onClick={onClose} />

      <aside className="
        fixed right-0 top-0 h-full w-full max-w-sm
        bg-gray-50 shadow-2xl z-[1000] flex flex-col animate-slide-in
      ">
        {/* Header */}
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2 className="text-base font-bold leading-tight truncate">{facility.name}</h2>
              </div>
              <p className="text-blue-200 text-xs">{facility.city} · {facility.county} County</p>
            </div>
            <button onClick={onClose} className="text-blue-200 hover:text-white transition-colors mt-0.5 shrink-0" aria-label="Close panel">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>
          <div className="mt-3"><TypeBadge type={facility.type} /></div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Ministry Events</h3>
              {canManage && (
                <button
                  onClick={() => setShowAddEvent(true)}
                  className="flex items-center gap-1.5 text-xs font-semibold bg-[#d69e2e] hover:bg-[#b7861f] text-white px-3 py-1.5 rounded-lg transition-colors shadow-sm"
                >
            ...
            {eventsLoading ? (
              ...
              <div className="text-center py-10 text-gray-400">
                ...
                {canManage && <p className="text-xs mt-1">Click <strong>Add Event</strong> to create one</p>}
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {events.map((event) => (
                  <EventCard key={event.id} event={event} facilityId={facilityId} />
                ))}
              </div>
            )}
          </div>
